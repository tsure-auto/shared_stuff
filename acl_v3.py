...
def iter_with_progress(
    ...
    - tqdm if available
    - otherwise fallback print every N items
    ...
    if _TQDM_AVAILABLE:
        return tqdm(iterable, total=total, desc=desc, unit="rule", mininterval=0.5)

    # Fallback: generator wrapper with periodic prints
    def _gen():
        for idx, item in enumerate(iterable, start=1):
            if fallback_every > 0 and (idx == 1 or idx % fallback_every == 0 or idx == total):
...
def main() -> int:
    ...
    # Progress controls
    ap.add_argument("--progress", action="store_true", help="Show progress bars (tqdm if available; fallback prints otherwise).")
    ap.add_argument("--progress-fallback-every", type=int, default=5000, help="If tqdm not installed, print progress every N rules (default 5000).")
    ...
    overall_report: Dict[str, Any] = {
        ...
        "settings": {
            ...
            "progress_fallback_every": args.progress_fallback_every,
    ...
    for acl_name, acl_rules in acls.items():
        ...
        summarized_rules, rep = summarize_acl_rules(
            ...
            fallback_every=args.progress_fallback_every,
        )
    ...
    progress_note(logger, f"WROTE summarized_json={out_json_path} report={report_path} log={log_path}")
    return 0
