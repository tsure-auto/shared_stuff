...
def summarize_acl_rules(
        ...
        if k in seen:
            ...
            if (prev.get("remark") or "") != (rule.get("remark") or ""):
                ...
                tracker.record("REMARK_DIFFER_ONLY", {
                    "acl": acl_name,
                    "kept_seq": prev.get("_orig_seq_key"),
                    "dropped_seq": seq_key,
                })
                logs.warning(
                    "REMARK_DIFFER_ONLY",
        ...
        for earlier in ip_permits_seen:
            if rule_covered_by_ip_permit(rule, earlier):
                ...
                tracker.record("FLAG_REDUNDANT_BY_IP_PERMIT", {
                    "acl": acl_name,
                    "seq": seq_key,
                    "covered_by_seq": earlier.get("_orig_seq_key"),
                })
                logs.warning(
            ...
            if current.get("dst_port_op") == "range" and nxt.get("dst_port_op") == "range" and can_merge_range(current, nxt):
                ...
                if candidate is not None:
                    ...
                    tracker.record("MERGE_RANGE_ADJACENT", {
                        ...
                        "base_seq": base_seq,
                        "merged_seq": next_seq,
                        "new_range": candidate.get("dst_ports"),
                    })
                    logs.info(
                        "RANGE_MERGED",
    ...
    merged_out = apply_exact_host_packing_contiguous(merged_out, acl_name, logger, logs, rep, tracker, vary_side="src", min_hosts=4)
    merged_out = apply_exact_host_packing_contiguous(merged_out, acl_name, logger, logs, rep, tracker, vary_side="dst", min_hosts=4)
    merged_out = apply_exact_subnet_packing_contiguous(merged_out, acl_name, logger, logs, rep, tracker, vary_side="src", min_subnets=2)
    merged_out = apply_exact_subnet_packing_contiguous(merged_out, acl_name, logger, logs, rep, tracker, vary_side="dst", min_subnets=2)
            ...
            for e in earlier_permits:
                if rule_covered_by_earlier_permit(r, e):
                    ...
                    tracker.record("FLAG_REDUNDANT_BY_BROADER_PERMIT", {
                        "acl": acl_name,
                        "rule_remark": r.get("remark"),
                        "covered_by_remark": e.get("remark"),
                    })
                    logs.warning(
